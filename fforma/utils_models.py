...
def _train_lightgbm(holdout_feats, best_models,
    ...
    if fobj is not None:
        if not gpboost:
            ...
            dtrain = lgb.Dataset(data=holdout_feats_train, label=indices_train, feature_name=holdout_feats_train.columns.tolist(), categorical_feature=['cat'])
            dvalid = lgb.Dataset(data=holdout_feats_val, label=indices_val, feature_name=holdout_feats_val.columns.tolist(), categorical_feature=['cat'])
        ...
        if not gpboost:
            ...
            dtrain = lgb.Dataset(data=holdout_feats_train, label=best_models_train, feature_name=holdout_feats_train.columns.tolist(), categorical_feature=['cat'])
            dvalid = lgb.Dataset(data=holdout_feats_val, label=best_models_val, feature_name=holdout_feats_val.columns.tolist(), categorical_feature=['cat'])
        valid_sets = [dtrain, dvalid]
