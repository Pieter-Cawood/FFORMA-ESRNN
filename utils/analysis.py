...
def evaluate_prediction_owa(predictions_df, y_train_df, y_test_df, naive2_seasonality, verbose=True, return_averages=True):
    y_panel = y_test_df.filter(['unique_id', 'ds', 'y'])
    y_naive2_panel = y_test_df.filter(['unique_id', 'ds', 'mdl_naive2'])
    y_naive2_panel.rename(columns={'mdl_naive2': 'y_hat'}, inplace=True)
    y_hat_panel = predictions_df
...
def smape(y, y_hat):
    ...
    """
    return 1 / len(y) * np.sum(2 * np.abs(y_hat - y) / (np.abs(y) + np.abs(y_hat)))
...
def evaluate_panel(y_panel, y_hat_panel, metric,
    ...
    evaluation = []
    for u_id in natsorted(y_panel.unique_id.unique()):
...
def owa(y_panel, y_hat_panel, y_naive2_panel, y_insample, seasonality, return_averages=True):
    """
    ...
    if return_averages:
        ...
        naive2_mase = np.array(total_mase_naive2)
        naive2_smape = np.array(total_smape_naive2) * 100
        model_mase = np.array(total_mase)
