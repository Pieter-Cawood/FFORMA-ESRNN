...
def m4_ts_parser(dataset_name, data_directory, load_existing_dataframes=True):
    """
    ...
    if load_existing_dataframes:
        try:
            X_train_df   = pd.read_pickle(f"{data_directory}/Preprocessed/{dataset_name}_X_train_df.pkl")
            y_train_df   = pd.read_pickle(f"{data_directory}/Preprocessed/{dataset_name}_y_train_df.pkl")
            X_test_df    = pd.read_pickle(f"{data_directory}/Preprocessed/{dataset_name}_X_test_df.pkl")
            y_test_df    = pd.read_pickle(f"{data_directory}/Preprocessed/{dataset_name}_y_test_df.pkl")
            with open(f"{data_directory}/Preprocessed/{dataset_name}_ts_pred_data.pkl", "rb") as f:
                ts_pred_data = pickle.load(f)
            return X_train_df, y_train_df, X_test_df, y_test_df, ts_pred_data
        except:
            print("Unable to load existing")
    ...
    X_train_df.to_pickle(f"{data_directory}/Preprocessed/{dataset_name}_X_train_df.pkl")
    y_train_df.to_pickle(f"{data_directory}/Preprocessed/{dataset_name}_y_train_df.pkl")
    X_test_df.to_pickle(f"{data_directory}/Preprocessed/{dataset_name}_X_test_df.pkl")
    y_test_df.to_pickle(f"{data_directory}/Preprocessed/{dataset_name}_y_test_df.pkl")

    y_train_df['unique_ID'] = pd.to_numeric(y_train_df.unique_id.str[1:], errors='coerce')
    ts_pred_data = dict((name, y_train_df.loc[y_train_df.unique_ID == name].y.to_numpy())
                        for name in y_train_df.unique_ID.unique())

    with open(f"{data_directory}/Preprocessed/{dataset_name}_ts_pred_data.pkl", "wb") as f:
        pickle.dump(ts_pred_data, f)
